...
void lidar_rawpoint_sub_cb(const sensor_msgs::PointCloud2ConstPtr &cloud_msg){
        ...
        box_filter.setMin(Eigen::Vector4f(searchPoint.x - 0.5, searchPoint.y - 0.2, -0.32, 1.0));//Min和Max是指立方体的两个对角点。每个点由一个四维向量表示，通常最后一个是1.（不知道为什么要有四个，大神知道的给解答一下下）
        box_filter.setMax(Eigen::Vector4f(searchPoint.x + 0.5, searchPoint.y + 0.2, 0.08, 1.0));

        boxplus_filter.setMin(Eigen::Vector4f(searchPoint.x - 1.0, searchPoint.y - 0.6, -0.5, 1.0));//Min和Max是指立方体的两个对角点。每个点由一个四维向量表示，通常最后一个是1.（不知道为什么要有四个，大神知道的给解答一下下）
        boxplus_filter.setMax(Eigen::Vector4f(searchPoint.x + 1.0, searchPoint.y + 0.6, 0.4, 1.0));
