...
void scanID::callback1(const sensor_msgs::PointCloud2ConstPtr &cloud_msg)
    {
            ...
            if (!laserRing[i].empty())    //判断该通道中是否有点云
            {
                ...
                for (int j = 0; j < laserRing[i].size(); j++)     //计算点到平面的距离
                 ...
                 if (!POINT_in_plane.empty())
                 {
                    ...
                    // 可以输出两条棱上的点坐标
                    // cout << "第一个点的x:" << POINT_in_plane[1].x << " " << "第一个点的y:" << POINT_in_plane[1].y <<  "第一个点的z:" << POINT_in_plane[1].z <<endl;
                    // cout << "最后一个点的x:" << POINT_in_plane[number_POINT_in_plane-1].x  << "最后一个点的y:" << POINT_in_plane[number_POINT_in_plane-1].y   << "最后一个点的z:" << POINT_in_plane[number_POINT_in_plane-1].z   << endl;
                    
                    point_distance = point_point_distance(POINT_in_plane[1],POINT_in_plane[number_POINT_in_plane-1]);
                    
                    if (point_distance > 0.36 && point_distance < 0.46)      //手持测试用：41cm
                    // if (point_distance > 0.33 && point_distance < 0.43)      //车上用：两条棱的距离38cm  阈值设置为33-43cm
                    {
                        ...
                        // cout<<"通道" << i << "上第一个点与最后一个点的距离:"<<point_to_point_dist<<endl;
                    }
                 }          
        ...
        // center.c_x = average_X;
        // center.c_y = average_Y;
...
int main (int argc, char** argv)
{
    ...
    scanID scanid;  // 实例化一个scanID类，会执行构造函数
    ...
    while (ros::ok())
    {
        ...
        //计算挂钩在后轴中心下的坐标
        // center.guagou_in_houzhouzhongxin_x = hook_in_erweima_x * sin(theta) + hook_in_erweima_y * cos(theta) + (-center.c_x) - rear_axle_center_in_lidar_y;
        // center.guagou_in_houzhouzhongxin_y = -(hook_in_erweima_x * cos(theta) - hook_in_erweima_y * sin(theta) + center.c_y - rear_axle_center_in_lidar_x);
